...
Generative probability models, in many ways the fundamental object of Bayesian machine learning, have long challenged researchers with the tradeoff between tractability and expressivity.
Though well understood that a model should be chosen to instantiate a set of assumptions and capture existing domain knowledge \cite{gelman2014bayesian, tenenbaum2006theory, mccullagh2002}, for many years too-simple models were chosen for practical advantanges (such as conditional conjugacy), which left much to be desired in terms of expressive performance and scalability of these models.  

More recently the pendulum has swung in the other direction, via models which map a latent random variable $z\sim p_0$ through a member of a highly expressive function family $\mathcal{G} = \left\{g_\theta : \theta \in \Theta\right\}$, the composition resulting in an \emph{implicit probability model} $\mathcal{M} = \left\{ g_\theta \circ z : \theta \in \Theta \right\}$.  
Choosing $\mathcal{G}$ is a deep neural network family has both a rich history \cite[, for example]{dayan1995helmholtz,mackay1997density}, and has recently been used to produce state-of-the-art results for density estimation \cite{uria2013rnade, rippel2013high, papamakarios2017masked}, generation of complex data \cite{Goodfellow:2014aa}, variational inference \cite{Kingma:2013aa, rezende2014stochastic, titsias2014doubly}, and many more.  A noted advantage of these models is that in many cases they make minimal assumptions about the data generative process.  This blessing is of course also a curse: since these models have been chosen to be generic and flexible, they can lack the classic stipulation that a model instantiates existing domain knowledge.



\cite{tenenbaum2006theory}
