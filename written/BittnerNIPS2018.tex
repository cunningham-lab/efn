...
\end{itemize}
 ...
 Fig 2.  This says, it works in a sensible case (maybe Dirichlet 25 or NIW 10x10):
 \begin{itemize}
 \item training and test curves (probably both of these are needless since by def train equals test here.
\item both in $r^2$ and in $KL$ to ground truth.
\item $MMD$ p values? Can this help give specifics? 
\end{itemize}

Fig 3.  This says, it works across a range of Dirichlet and NIW (and perhaps not Gaussian).  Sensitivities
\begin{itemize}
\item dimensionality of dir and NIW
\item choices of $K,M$ (is this really needed)
\item key comparison to baseline of VI (doing this specifically for K=1 but a fixed K, possibly with no parameter network because it's unnecessary).  The key is to show that it works almost as well as VI in the specific case, but of course VI can't generalize.  And when I say VI I don't really mean VI, I mean just learning a specific member of the exp fam.    
\end{itemize}

Fig 4.  One great virtue of EFNs is is learning a restricted model, which should demonstrate the usual bias-variance tradeoff.  We can show this.  Suppose we have only a limited amount of data from a specific exp fam, but we don't know what member.  Yeah this is cool.  So we can either explore all of $\phi$ space in the usual way (the full flow network model $\mathcal{Q}$), or we can have previously learned the exp fam, which operates on the restricted coordinates of the natural parameters.  Then, we take gradients over natural parameter space to find the best $\phi$, which is really $\phi^* = \phi(\eta^*)$, and hopefully that distribution is closer in KL to the true posterior than had we just learned straight from $\mathcal{Q}$.  Well this isn't quite fair, as we don't even need to take gradients in $\eta$ space, since in the simplest case we just plug in $\eta$ and we've got the exact member.  But yes that's a good example too.  Possibly better.  As a function of available data, can you fit the posterior?  Well, you can learn directly from $\mathcal{Q}$, always an option.  That shows something probably unhelpful: that you can learn a point mass or something like that.  Hmmm not quite.  Ok use a Norm Flow with a bayesian logistic regression generative model and limited data.  Learn the posterior on $\beta$ for a given dataset.  That maps a dataset $X$ to a posterior.  That will run into scaling problems as $X$ grows, because apparently the network has to follow suit.  Instead cheat and give it some sufficient statistics.  And ok that's not so much of a cheat because you would know from the model.  So that's fair I suppose.  Another choice is to just model the data, so write the generative model and ask the norm flow to learn the posterior.  That's the same thing and I'm talking in circles.  There is a good idea here.

\emph{Chapter 3: Fig 5 and 6}

Fig 5. Bayesian logistic regression.  Learn the full BLR posterior (BLRP) family for some dimensionality.  Then get some data (see links in slack, look for nickish and all that).  Then find the posterior.  etc, etc, etc.  Then use MF VI or EP or whatever in an attempt to do same.  Show outperformance.  Really scrutinize the comparison point here.  What really is VI in this setting?  First, what is bayesian logistic regression.  

$$ \beta \sim \mathcal{N}(0,I) ~~~~~ y | \beta, x  \sim_{iid} Bern\left( \frac{1}{1 + \exp\left\{ \beta^\top x \right\} } \right) $$

In our nomenclature, $\beta$ is $z$, $y,x$ are some variables that will figure into the natural parameters $\eta$.  In BLR, we are interested in the posterior on $\beta$ given the dataset $\{X,Y\}$:

$$ p(\beta | \{Y,X\}) = \frac{ p(\beta, \{Y,X\})}{p(\{Y,X\})} = \frac{ p(\{Y,X\}|\beta) p(\beta)}{p(\{Y,X\})} = \frac{ p(\{Y,X\}|\beta) \exp\left\{\eta_0^\top t_0(\beta)\right\}}{p(\{Y,X\})}$$

Ugh.  BLR is an intractable exp fam only for a particular set of $X$, making it rather a crappy choice.  Groan.

Fig 5.  Latent Dirichlet Allocation?  Some other similar problem? Nope that doesn't work either
...
\item check MKB book for other cool MV distributions.
...
\item Ising models: classic, bw images
