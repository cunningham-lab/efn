...
swapping $\beta$ for $\gamma$ and using $1^\top$ and some \begin{verbatim}\left, \right\end{verbatim}...

$$
p(\pmb{z} \mid X) \propto \exp\left\{\begin{bmatrix} \pmb{\alpha}_0 - \pmb{1} \\ \beta  \sum_{i=1}^N \log(\pmb{x}_i) \end{bmatrix}^\top \begin{bmatrix} \log(\pmb{z}) \\ \pmb{z} \end{bmatrix} \right\}  ~~\triangleq~~ \exp\left\{ \pmb{\eta}^\top t(\pmb{z}) \right\}
$$

...or without the bold (and $z-1$ or $z$ as you say doesn't matter)...

$$
p(z \mid X) \propto \exp\left\{\begin{bmatrix} {\alpha}_0 - {1} \\ \beta  \sum_{i=1}^N \log({x}_i) \end{bmatrix}^\top \begin{bmatrix} \log({z}) \\ {z-1} \end{bmatrix} \right\}  ~~\triangleq~~ \exp\left\{ {\eta}^\top t({z}) \right\}
$$
