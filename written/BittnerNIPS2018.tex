...
More recently the pendulum has swung in the other direction, via models which map a latent random variable $z\sim p_0$ through a member of a highly expressive function family $\mathcal{G} = \left\{g_\theta : \theta \in \Theta\right\}$, the composition resulting in an \emph{implicit probability model} $\mathcal{M} = \left\{ g_\theta \circ p_0 : \theta \in \Theta \right\}$.
