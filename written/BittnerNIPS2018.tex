...
We will focus on perhaps the most fundamental problem setup in probabilistic inference, that of a latent variable $z \in \mathcal{Z}$ with prior belief $p(z)$, and where we observe a dataset $X = \left\{x_1,...,x_N\right\} \subset \mathcal{X}$ as conditionally independent draws given $z$.   Updating our belief with data produces the posterior $p(z | X) \propto p(z) \prod_{i=1}^N p(x_i | z)$.  This setup is shown as a graphical model in Figure 1A.
...
Of course, in all but the most simple cases, this posterior is intractable.  Here it is important to be explicit in our definition of intractability: typically due to a lack of known conjugacy, this posterior falls outside the class of models that can be readily sampled or evaluated given existing numerical techniques (for example.  However, 
