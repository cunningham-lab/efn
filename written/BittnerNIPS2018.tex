...
KL p q for a fixed eta

Note that this objective can also produce approximations of the log partition, via essentially linear regression; more nuanced schemes are likely appropriate \cite{papamakarios2015distilling}.  We don't explore that here.

Of course we don't just want to learn a single eta, but the whole model.  As a practical necessity we introduce a sampling distribution... and we are interested in the $E(KL)$, or equiv KL(...)

This is amenable to sampling, and note that since we have no data (we are fitting a model, not a conditional based on finite data), we can sample forever.  Here is an unbiased objective for stochastic optimization.


This defines an EFN.  What remains for empirical implementation is to make particular choices of hyperparameters, network layers, and optimization algorithm, which we specify in \S3 below.
