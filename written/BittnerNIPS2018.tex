...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \subsection{Density networks as approximating family $\mathcal{Q}$}

Implicit probability models, which we will use for our approximating family $\mathcal{Q}$, can be defined by any base random variable $w\sim p_0$ mapped through any measurable, parameter-indexed function family  $\mathcal{G} = \left\{g_\theta: \theta \in \Theta\right\}$; we denote the induced density on $z=g_\theta(w)$ as $q_\theta(z)$.   
