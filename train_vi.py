...
def train_vi(exp_fam, D, flow_dict, cost_type, M_eta=1000, model_info={}, \
    ...
    savedir = setup_IO(exp_fam, K_eta, M_eta, D, flow_dict, {}, model_info, stochasticEta, False, random_seed);
    eta = tf.placeholder(tf.float64, shape=(None, ncons));
    Tx_input = tf.placeholder(tf.float64, shape=(None, num_Tx_inputs));

    _eta_test, _param_net_input_test, _Tx_input_test, eta_test_draw_params = drawEtas(exp_fam, D, 100, model_info, False, test=True);
    ...
    with tf.Session() as sess:
        ...
        _eta, _param_net_input, _Tx_input, eta_draw_params = drawEtas(exp_fam, D, K_eta, model_info, False);
        ...
        while (i < max_iters):
            ...
            if (np.mod(i+1, check_rate)==0):
                ...
                train_costs_i, train_R2s_i, train_KLs_i = batch_diagnostics(exp_fam, K_eta, sess, feed_dict_train, X, log_p_zs, costs, R2s, {});
                test_costs_i, test_R2s_i, test_KLs_i = batch_diagnostics(exp_fam, K_eta, sess, feed_dict_test, X, log_p_zs, costs, R2s, {});
                ...
                if (dynamics):
                    np.savez(savedir + 'results.npz', As=As, sigma_epsilons=sigma_epsilons, autocov_targ=autocov_targ,  \
                                                      ...
                                                      eta_test_draw_params=eta_test_draw_params, check_rate=check_rate, \
                                                      train_elbos=train_elbos, test_elbos=test_elbos, \
                                                      train_R2s=train_R2s, test_R2s=test_R2s, \
                                                      train_KLs=train_KLs, test_KLs=test_KLs);
                else:
                    np.savez(savedir + 'results.npz', it=i, X=_X, eta=_eta, Tx_input=_Tx_input, eta_draw_params=eta_draw_params, \
                                                      eta_test_draw_params=eta_test_draw_params, check_rate=check_rate, \
                                                      train_elbos=train_elbos, test_elbos=test_elbos, \
                                                      train_R2s=train_R2s, test_R2s=test_R2s, \
                                                      train_KLs=train_KLs, test_KLs=test_KLs);
